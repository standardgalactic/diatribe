...
summary_file="diatribe-research.txt"
main_dir=$(pwd)
    ...
    for file in "$dir"/*.txt; do
        ...
        if [ -f "$file" ]; then
            ...
            if ! is_processed "$file_name"; then
                ...
                # Split the file into chunks of 2500 lines each
                split -l 2500 "$file" "$temp_dir/chunk_"
		...
		for chunk_file in "$temp_dir"/chunk_*; do
			[ -f "$chunk_file" ] || continue
				echo "Summarizing chunk: $(basename "$chunk_file")"
					ollama run granite3.3:8b "Summarize in detail and explain:" < "$chunk_file" | tee -a "$main_dir/$summary_file"
					echo "" >> "$main_dir/$summary_file"
					done
